...
module.exports = class Homenet {
	constructor(port){
		this.boards = [];
		this.devices = [];
		this.boardDB = new Datastore({filename: './var/'+process.env.PLATFORM_NAME+'_board.db', autoload: true});
		this.boardDB.ensureIndex({ fieldName: 'mac', unique: true }, function (err) {
			if (err) console.error("Unable to set index on mac", err);
		});
		this.deviceDB = new Datastore({filename: './var/'+process.env.PLATFORM_NAME+'_device.db', autoload: true});
